...
TerrainWidget::~TerrainWidget()
{
    ...
    if (cellDecomp != nullptr) delete cellDecomp;
}
...
void TerrainWidget::setHeightfield(const HeightField& hf, bool centerCamera)
{
    ...
    std::cout << bbox.getMin()[0] << " " << bbox.getMin()[1] << " " << a << std::endl;
    std::cout << bbox.getMax()[0] << " " << bbox.getMax()[1] << " " << b << std::endl;
...
void TerrainWidget::setDecomposition(CellDecomposition* decomp, HeightField* hf, double phi, double theta) {
    delete cellDecomp;
    cellDecomp = decomp; 
    ...
    eroder.setParams(phi, theta, Vector3(0,0,-1));
}


void TerrainWidget::changeRenderCells() {
    renderCells = !renderCells;
    update();
}

void TerrainWidget::changeRenderParticles() {
    renderParticles = !renderParticles;
    update();
}

void TerrainWidget::changeRenderOriginal() {
...
void TerrainWidget::refreshErosionParams(double phi, double theta) {
    eroder.setParams(phi, theta);
}
