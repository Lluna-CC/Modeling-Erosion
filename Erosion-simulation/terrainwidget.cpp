...
void TerrainWidget::setToyDecomposition(CellDecomposition* decomp, double theta, double phi) {
    delete cellDecomp;
    cellDecomp = decomp; 
...
void TerrainWidget::computeStress() {
    if (cellDecomp != nullptr) eroder.computeStress();
}
...
void TerrainWidget::exportAsPoly() {
    if (cellDecomp != nullptr) cellDecomp -> exportAsPLY();
}