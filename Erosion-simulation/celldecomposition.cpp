...
void CellDecomposition::fullMeshDecomposition(int l) {
    ...
    for (int i = 0; i < cells.size(); ++i) {
        ...
        cellToMesh(cells[i].vertices, cells[i].faceData, cellVAOs[l][i], bufferVerts[l][i], bufferIndices[l][i], cells[i].nTriangles, cellIndices[i]);
    }
...
void CellDecomposition::cellToMesh(std::vector<float>& v, std::map<int,voroFace>& f, GLuint& meshVAO, GLuint& bufferVerts, GLuint& bufferIndices, int& triangles, std::vector<unsigned int>& indices) {
    indices.clear();
...
void CellDecomposition::updateMesh(const std::vector<int>& newC, const std::vector<int>& oldC) {
    ...
    for (int i = 0; i < newC.size(); ++i) {
        ...
        cellToMesh(cells[k].vertices, cells[k].faceData, cellVAOs[0][k], bufferVerts[0][k], bufferIndices[0][k], cells[k].nTriangles, cellIndices[k]);
    }
...
void CellDecomposition::voronoiDecomposition(std::vector<float>& v, std::vector<uint>& f, const HeightField *hf, int multiRes_factor, 
    ...
    else graph.multiLevelVoronoiDecompositionGridSampling(v,f,hf, 3, core_center, core_range, zSamples);
}

bool CellDecomposition::exportAsPLY() {
    
    std::vector<vorocell>& cells = graph.getCells(0);
    int nVerts = 0, nTris = 0;
    for (int i = 0; i < cells.size(); ++i) {
        if (cells[i].state == AIR || cells[i].state == DISCARDED) continue;
        nVerts += cells[i].vertices.size()/3;
        nTris += cells[i].nTriangles;
    }

    std::ofstream ofs;
    std::string filename = "./PLYTests/exported.ply";
    ofs.open(filename);
    ofs << "ply\n";
    ofs << "format binary_little_endian 1.0\n";
    ofs << "element vertex " + std::to_string(nVerts) + "\n";
    ofs << "property float x\n";
    ofs << "property float y\n";
    ofs << "property float z\n";
    ofs << "element face " + std::to_string(nTris) + "\n";
    ofs << "property list uchar uint vertex_indices\n";
    ofs << "end_header\n";
    char const * buff;
    unsigned char triangles_per_face = 3;
    for (int i = 0; i < cells.size(); ++i) {
        ...
        for (int j = 0; j < cells[i].vertices.size(); j += 3){
            ...
            buff = reinterpret_cast<char const *>(&cells[i].vertices[j]);
            ofs.write(buff,sizeof(float));
            buff = reinterpret_cast<char const *>(&cells[i].vertices[j + 1]);
            ofs.write(buff,sizeof(float));
            buff = reinterpret_cast<char const *>(&cells[i].vertices[j + 2]);
            ofs.write(buff,sizeof(float));
    ...
    for (int i = 0; i < cellIndices.size(); ++i) {
        if (cells[i].state == AIR || cells[i].state == DISCARDED) continue;
        for (int j = 0; j < cellIndices[i].size(); j += 3) {
            buff = reinterpret_cast<char const *>(&triangles_per_face);
            ofs.write(buff,sizeof(unsigned char));


          
            int vert = cellIndices[i][j] + actVert;
            buff = reinterpret_cast<char const *>(&vert);
            ofs.write(buff,sizeof(unsigned int));    

            vert = cellIndices[i][j + 1] + actVert;
            buff = reinterpret_cast<char const *>(&vert);
            ofs.write(buff,sizeof(unsigned int));    
            
            vert = cellIndices[i][j + 2] + actVert;
            buff = reinterpret_cast<char const *>(&vert);
            ofs.write(buff,sizeof(unsigned int));    

        }
        actVert += cells[i].vertices.size()/3;
    }
