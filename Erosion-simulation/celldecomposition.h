...
class CellDecomposition: protected QOpenGLFunctions_3_3_Core, QOpenGLContext {
 ...
 private:
   ...
   std::vector<std::vector<unsigned int>> cellIndices;
   ...
   void cellToMesh(std::vector<float>& v, std::map<int,voroFace>& f, GLuint& meshVAO, GLuint& bufferVerts, GLuint& bufferIndices, int& triangles, std::vector<unsigned int>& indices);
   void computeBounds();  
